...
import useNavigationLoader from '../utils/useNavigationLoader';
...
const Dashboard = () => {
    ...
    const isNavigating = useNavigationLoader(200); // Show loader for 200ms during navigation
    ...
    useEffect(() => {
        // Reduced delay for faster loading - token should be available immediately
        setTimeout(() => {
            refresh();
        }, 100);
    ...
    // Render themed skeleton loading state
    const renderSkeletons = () => (
        <Box sx={{ p: 3, bgcolor: 'background.default', minHeight: '100vh' }} className="fade-in">
            {/* Header skeleton */}
            <Box sx={{ mb: 4 }}>
                <Skeleton 
                    variant="text" 
            ...
            <Grid container spacing={3} sx={{ mb: 4 }}>
                {[...Array(4)].map((_, index) => (
                    <Grid item xs={6} sm={6} md={3} key={`skeleton-top-${index}`}>
                        <Card sx={{
                            height: 140,
                            ...
                            <CardContent sx={{ p: 3 }}>
                                ...
                                <Skeleton 
                                    ...
                                    height={40} 
                                    sx={{ 
                                        bgcolor: theme.palette.mode === 'dark' ? 'rgba(255, 255, 255, 0.15)' : 'rgba(0, 0, 0, 0.15)',
                                        mb: 1
                                    }} 
                                />
                                <Skeleton 
